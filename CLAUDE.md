...
**`daily_sliders.feelings` is free text, not a DB enum; `practice_log` is an orphaned legacy column.** `feelings` (primary influencing-factor label) is a plain `TEXT` column with no `CHECK` constraint — its option list lives entirely in `mobile/src/screens/roadmap/DailySlidersScreen.tsx` (`INFLUENCING_FACTORS`); changing it is a frontend-only change, no migration needed. `practice_log` was superseded by `mindfulness_practice`/`practice_duration`/`practice_location` (the mobile client's `PRACTICE_LOCATIONS` list backs `practice_location`) and nothing writes to it anymore — it's dead schema, not an active free-text field. `mindfulness_practice` and `practice_location` ARE constrained via `CHECK` at the DB level, as are the numeric scales (`stress_level`, `calm_before`, `calm_after`, `sleep_quality`) 1–5.
