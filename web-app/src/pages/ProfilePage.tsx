...
export default function ProfilePage() {
  ...
  if (loading) {
    return (
      ...
      <div style={{ minHeight: '100vh', background: '#F8FAF8', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ width: 36, height: 36, border: '3px solid #E3F2FD', borderTopColor: '#0F9B71', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
  ...
  return (
    ...
    <div style={{ minHeight: '100vh', background: '#F8FAF8', paddingBottom: 80 }}>
      {/* Header */}
      <div style={{ background: '#0F9B71', paddingTop: 'env(safe-area-inset-top, 0px)', padding: '20px 20px 40px' }}>
      ...
      <div style={{ maxWidth: 430, margin: '-16px auto 0', padding: '0 16px' }}>
        ...
        <div style={{ background: '#fff', borderRadius: 16, padding: 20, marginBottom: 12, boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
          ...
          {profile.research_id ? (
            ...
            <div style={{ background: '#FFF6E5', borderRadius: 12, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ fontSize: 18 }}>⏳</span>
              <p style={{ fontSize: 13, color: '#EA8F00', fontWeight: 500 }}>Researchers will add you to a group shortly.</p>
            </div>
          )}
        </div>
        ...
        <div style={{ background: '#fff', borderRadius: 16, overflow: 'hidden', marginBottom: 12, boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ width: 36, height: 36, borderRadius: 10, background: '#E7F9F1', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#0F9B71' }}><User size={17} /></div>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ width: 36, height: 36, borderRadius: 10, background: '#E8F0FE', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#3E7BFA' }}><Lock size={16} /></div>
              <div style={{ textAlign: 'left' }}>
      ...
      {resetStep === 'step1' && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: 24 }}>
          <div style={{ background: '#fff', borderRadius: 24, padding: 28, maxWidth: 340, width: '100%' }}>
            ...
            <div style={{ background: '#F8FAF8', borderRadius: 12, padding: '12px 16px', marginBottom: 12 }}>
              <input
                value={resetEmail}
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              <button onClick={() => setResetStep('closed')} style={{ flex: 1, padding: 14, background: '#F8FAF8', color: '#636E72', border: 'none', borderRadius: 12, fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
              <button onClick={sendResetCode} disabled={resetLoading} style={{ flex: 1, padding: 14, background: '#0F9B71', color: '#fff', border: 'none', borderRadius: 12, fontWeight: 700, cursor: 'pointer', opacity: resetLoading ? 0.6 : 1 }}>
      ...
      {resetStep === 'step2' && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: 24, overflowY: 'auto' }}>
          <div style={{ background: '#fff', borderRadius: 24, padding: 28, maxWidth: 340, width: '100%' }}>
              ...
              {resetDigits.map((d, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center' }}>
                  <input
                    ...
                    style={{ width: 32, height: 40, textAlign: 'center', fontSize: 18, fontWeight: 700, border: `2px solid ${d ? '#0F9B71' : '#DFE6E9'}`, borderRadius: 8, outline: 'none', background: d ? '#E7F9F1' : '#fff' }}
                  />
                  {i === 3 && <div style={{ width: 8 }} />}
                </div>
              ))}
            </div>
            <div style={{ background: '#F8FAF8', borderRadius: 12, padding: '12px 16px', marginBottom: 10 }}>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              <button onClick={() => setResetStep('closed')} style={{ flex: 1, padding: 14, background: '#F8FAF8', color: '#636E72', border: 'none', borderRadius: 12, fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
              <button onClick={confirmReset} disabled={resetLoading} style={{ flex: 1, padding: 14, background: '#0F9B71', color: '#fff', border: 'none', borderRadius: 12, fontWeight: 700, cursor: 'pointer', opacity: resetLoading ? 0.6 : 1 }}>
